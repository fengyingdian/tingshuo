...
Page({
  ...
  streamRecord: function(e) {
    ...
    var idInterval = setInterval(function () {
      ...
      wx.showToast({
        title: that.data.recordSecondCount.toString(),
      })
    }.bind(that), 1000)
    that.setData({
  ...
  streamRecordEnd: function(e) {
    ...
    app.RRMErrorCallback = res => {
      console.log("tran.manager.onError")
      this.setData({
        recording: false,
    ...
    //有新的识别内容返回，则会调用此事件
    app.RRMRecognizeCallback = res => {
      console.log("tran.manager.onRecognize")
    ...
    app.RRMStopCallback = res => {
      console.log("tran.manager.onStop")
      let text = res.result
      if (text == '') {
